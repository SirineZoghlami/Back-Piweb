...
export const createFactureSteg = async (req, res) => {
  try {
    ...
    res.status(201).json({ message: 'FactureSteg créée avec succès', factureSteg });
  } catch (error) {
...
export const getAllFacturesSteg = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Échec de la récupération des facturesSteg' });
  }
};
...
export const getFactureStegById = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Échec de la récupération de la factureSteg' });
  }
};
...
export const updateFactureStegById = async (req, res) => {
  const { id } = req.params;
  try {
    const updatedFactureSteg = await FactureSteg.findByIdAndUpdate(id, req.body, { new: true });
    if (!updatedFactureSteg) {
      return res.status(404).json({ error: 'FactureSteg non trouvée' });
    }
    res.status(200).json({ message: 'FactureSteg mise à jour avec succès', factureSteg: updatedFactureSteg });
  } catch (error) {
    console.error('Erreur lors de la mise à jour de la factureSteg :', error);
    res.status(500).json({ error: 'Échec de la mise à jour de la factureSteg' });
  }
};
...
export const deleteFactureStegById = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'FactureSteg supprimée avec succès', factureSteg: deletedFactureSteg });
  } catch (error) {
    console.error('Erreur lors de la suppression de la factureSteg :', error);
    res.status(500).json({ error: 'Échec de la suppression de la factureSteg' });
  }
};
